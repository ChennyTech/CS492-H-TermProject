...
from modules.keypoint_detector import gaussian2kp_3d, gaussian2kp
import cv2
import numpy as np
...
def split_kp(kp_joined, detach=False):
    ...
    # kp_video['mean']: B 1 10 2, -1~1 // ['var']: B, 1, 10, 2, 2
...
class GeneratorFullModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        kp_joined = self.kp_extractor(torch.cat([x['source'], x['video']], dim=2)) # B 2 10 2 // B, 2, 10, 2, 2 /// # B 2 10 3 // B, 2, 10, 3, 3

        generated = self.generator(x['source'],
                                   **split_kp(kp_joined, self.train_params['detach_kp_generator']))
        kp_dict = split_kp(kp_joined, False) # shape check
        ...
        if self.opt.is_original:
            kp_dict = split_kp(kp_joined, True) # for discriminator

        return tuple(losses) + (generated, kp_dict)
...
class DiscriminatorFullModel(torch.nn.Module):
    ...
    def forward(self, x, kp_dict, generated):
...
def train(config, generator, discriminator, kp_detector, checkpoint, log_dir, dataset, device_ids, opt):
    ...
    generator_full = GeneratorFullModel(kp_detector, generator, discriminator, train_params, opt)
