...
def transfer_one(generator, kp_detector, source_image, driving_video, transfer_params, is_3d= False):
    ...
    if is_3d:
        cat_dict = lambda l, dim: {k: torch.cat([v[k] for v in l], dim=dim) for k in ['video_prediction']}
...
def transfer(config, generator, kp_detector, checkpoint, log_dir, dataset, opt):
    ...
    for it, x in tqdm(enumerate(dataloader)):
        with torch.no_grad():
            ...
            out = transfer_one(generator, kp_detector, source_image, driving_video, transfer_params, is_3d = opt.is_3d)
