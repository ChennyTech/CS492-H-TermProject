...
class SplitSourceDriving(object):
    def __call__(self, video):
        resized_source = cv2.resize(video[:1][0], dsize=(64, 64), interpolation=cv2.INTER_CUBIC)
        resized_video = cv2.resize(video[1:][0], dsize=(64, 64), interpolation=cv2.INTER_CUBIC)
        source = np.array(np.expand_dims(resized_source,0), dtype='float32')
        video = np.array(np.expand_dims(resized_video,0), dtype='float32')
...
class VideoToTensor(object):
    ...
    def __call__(self, driving):
        ...
        for i in range(driving.shape[0]):
            resized_driving.append(cv2.resize(driving[i], dsize=(64, 64), interpolation=cv2.INTER_CUBIC))
        driving = np.stack(resized_driving, axis=0)
