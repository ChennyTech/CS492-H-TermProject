...
class Visualizer:
    ...
    def visualize_transfer(self, driving_video, source_image, out):
        ...
        kp_video = out['kp_driving']['mean'].data.cpu().numpy()[:,:,:,0:2]
        kp_appearance = out['kp_source']['mean'].data.cpu().repeat(1, out_video_batch.shape[2], 1, 1).numpy()[:,:,:,0:2]
        kp_norm = out['kp_norm']['mean'].data.cpu().numpy()[:,:,:,0:2]
        kp_video_first = out['kp_driving']['mean'][:, :1].data.cpu().repeat(1, out_video_batch.shape[2], 1, 1).numpy()[:,:,:,0:2]
    ...
    def visualize_reconstruction(self, inp, out):
        ...
        kp_video = out['kp_driving']['mean'].data.cpu().numpy()[:,:,:,0:2] # original 2 // original 1은 1:3
        kp_appearance = out['kp_source']['mean'].data.cpu().repeat(1, out_video_batch.shape[2], 1, 1).numpy()[:,:,:,0:2]
        ...
        image = self.create_image_grid((appearance_video_batch, kp_appearance), (gt_video_batch, kp_video),
                                       out_video_batch,gt_video_batch)
